...
class WalletLabelsTest(BitcoinTestFramework):
    ...
    def _run_subtest(self, accounts_api, node):
        ...
        node.generate(1)
        node.generate(101)
        assert_equal(node.getbalance(), 600000)
        ...
        for address_group in address_groups:
            ...
            assert_equal(address_group[0][1], 300000)
        ...
        for label in labels:
            ...
            label.verify(node)
        node.generate(101)
        expected_account_balances = {"": 31200000}
        for label in labels:
            expected_account_balances[label.name] = 0
        if accounts_api:
            assert_equal(node.listaccounts(), expected_account_balances)
            assert_equal(node.getbalance(""), 31200000)
        ...
        for label in labels:
            ...
            label.verify(node)
            if accounts_api:
                node.sendfrom("", multisig_address, 300000)
        node.generate(101)
        if accounts_api:
            for label in labels:
                assert_equal(node.getbalance(label.name), 300000)
